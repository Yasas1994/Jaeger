...
def load_model(config_path, checkpoint_path=None):
    ...
    if "training" in config:
        ...
        checkpoint_dir = checkpoint_dir.replace(
            "{{ model.base_dir }}", config["model"]["base_dir"]
        )
        checkpoint_dir = checkpoint_dir.replace(
            "{{ training.experiment_root }}",
            f"experiments/experiment_{config['model']['experiment']}_{config['model']['seed']}",
        )
...
def main():
    ...
    np.savez(
        output_dir / f"{args.model_name}_results.npz",
