...
from typing import Any, Dict, DefaultDict, Generator, List, Optional
import yaml
...
class DynamicInferenceModelBuilder:
    ...
    def build_fragment_classifier(self):
        ...
        models["jaeger_model"] = JaegerModel(inputs=models["rep_model"].input, 
...
class InferModel:
    ...
    def __init__(self, path_dict):
        ...
        self.string_processor_config = self._load_string_processor_config(path_dict.get("project", None))
    ...
    def predict(self, dataset, no_progress: bool = False) -> dict[str, np.ndarray]:
        ...
        for inputs, *meta in track(dataset, description="[cyan]Crunching data…", disable=no_progress):
    ...
    def evaluate(self, dataset, no_progress: bool = False) -> dict[str, float]:
        ...
        for inputs, y_true in track(dataset, description="[cyan]Evaluating…", disable=no_progress):
    ...
    def _load_string_processor_config(self, path):
        ...
        if path:
            ...
            return {'input_type':cfg.get('model').get('embedding').get('type'),
                    ...
                    'codon': _map.get(cfg.get('model').get('string_processor').get('codon')),
                    'codon_id': _map.get(cfg.get('model').get('string_processor').get('codon_id'))
                    }
        return None