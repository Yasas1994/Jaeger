    ...
    with open(output_path, "w") as fh:
        for name, seq in track(f, description="Processing..."):
            ...
            while current_perc <= max_perc:
                ...
                if not available:
                    break
                new_indices = np.random.choice(available, min(num_mutate, len(available)), replace=False)
...
def dataset_core(**kwargs):
    ...
    if shutil.which("mmseqs") is None:
        sys.exit("Error: MMseqs2 (`mmseqs`) not found in PATH. Please install MMseqs2 or add it to your PATH.")
            ...
            if L >= FRAG_LEN:
                while start < L:
                    ...
                    if end == L:
                        break
                    start = end - OVERLAP
    num_frags = len([name for i,_ in pyfastx.Fasta(str(frag_fasta), build_index=False)])
    ...
    subprocess.run(
        [
            "mmseqs", "easy-cluster",
            frag_fasta, clusters, tmpdir,
            "--min-seq-id", str(maxiden),
            "-c", str(maxcov)
        ],
        check=True,
    )
    ...
    print(f"{num_frags} frags →  {len(reps)} reps → "
