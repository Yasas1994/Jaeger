...
def get_logger(log_path:Path, log_file:str, level:int) -> logging.Logger:
    ...
    logger.setLevel(loglevel)  # Set minimum level (DEBUG, INFO, WARNING, ERROR, CRITICAL)

    # Create handlers
    console_handler = logging.StreamHandler(sys.stderr)  # Logs to console
    file_handler = logging.FileHandler(log_path/Path(f"{current_date_time}_{log_file}"))        # Logs to a file
    ...
    console_handler = RichHandler(show_time=False, omit_repeated_times=False, show_level=False, show_path=False, enable_link_path=False )
