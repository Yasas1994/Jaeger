...
class HighWay(keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        y = tf.add(tf.mul(H, T), tf.mul(x, C), "hw-out")
...
class CustomModel(tf.keras.Model):
    ...
    def compile(self, optimizer):
        ...
        self.precision_tracker = keras.metrics.Mean(name="precision") #keeps track on current precision 
        self.recall_tracker = keras.metrics.Mean(name="recall") #keeps track on current precision 
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape() as tape:
            y_logits = self(x, training=True)  # Forward pass
            y = tf.cast(y, dtype=y_logits.dtype) # cast to float32
            # Compute our own loss
            loss=tf.nn.softmax_cross_entropy_with_logits(labels=tf.cast(y, dtype=y_logits.dtype), \
                                                         logits= tf.squeeze(y_logits), \
                                                         name='train-loss')
        ...
        #y_pred = tf.keras.activations.softmax(y_logits)
        # Compute our own metrics
        self.loss_tracker.update_state(loss)
        # Compute probabilities from logits 
        y_pred = tf.keras.activations.softmax(y_logits)
        pre = precision_m(y, y_pred)
        rec = recall_m(y, y_pred)
        self.precision_tracker.update_state(pre)
        self.recall_tracker.update_state(rec)
